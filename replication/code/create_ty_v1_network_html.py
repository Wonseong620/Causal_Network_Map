...
function networkTitle(language) {{
  return `${{language}} Media Lead-Lag by Sectors`;
}}

function render() {{
  const language = controls.language.value;
  const titleText = networkTitle(language);
  ...
  svg.appendChild(el("text", {{ x: 500, y: 46, class: "title" }}, titleText));
...
function downloadPng() {{
  ...
  style.textContent = `
    ...
    .title {{ font-size: 18px; font-weight: 800; text-anchor: middle; font-family: Arial, Helvetica, sans-serif; }}
