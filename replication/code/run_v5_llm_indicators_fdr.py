# -*- coding: utf-8 -*-
"""Phase 1+2 on LLM-classified probabilities (Phase 4 output).

Merges llm_probs_full_<lang>.csv (orig_index-keyed, semantic classification)
with the original article metadata (published_at, negativity, war_related),
then repeats the v4 pipeline: within-day share transform, PC1 diagnostic,
TY all-pairs test with Benjamini-Hochberg FDR, and aggregate V/H indicators.

No uniform-row exclusion is needed here (Phase 4 produced 0 uniform rows),
but the share transform still matters: LLM mass is concentrated (public
administration ~35%), so daily volume swings during hostility spikes would
otherwise still inject a common-factor artifact into the raw series.

Output -> figures/causal_network_v5_llm_fdr/
"""
import sys, io, time
...
OUT = BASE / "figures" / "causal_network_v5_llm_fdr"
...
for lang, fname in LANGS.items():
    t0 = time.time()
    print(f"=== {lang} ===")
    meta = pd.read_csv(BASE / fname, usecols=["published_at", "negativity", "war_related"],
                       low_memory=False)
    meta["published_at"] = pd.to_datetime(meta["published_at"], errors="coerce", dayfirst=True).dt.floor("D")
    llm = pd.read_csv(LLM_DIR / f"llm_probs_full_{lang}.csv")
    llm = llm.set_index("orig_index")
    ...
    print(f"merged articles={len(df)} (llm={len(llm)}, meta={len(meta)})")

    # ---- aggregate indicators (all rows contribute; no uniform rows) ----
    V = P.sum(axis=0)
    H = P.mul(neg * war, axis=0).sum(axis=0)
    ind = pd.DataFrame({
        "V1": range(1, 51),
    ...
    print(f"corrected series: PC1={pc1:.1f}%  mean|r|={mabsr:.3f}  active_sectors={active.sum()}")
    ...
    edges.to_csv(OUT / f"ty_edges_allpairs_llm_{lang}.csv", index=False)
